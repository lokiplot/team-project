...
class Group:
    ...
    def update_data(self, new_one, cell_to_update):
        ...
        cur.execute(f'UPDATE {name} SET '
                    f'average_percent={values[1]}, '
                    f'archive1={values[2]}, '
                    f'archive2={values[3]}, '
                    f'archive3={values[4]}, '
                    f'archive4={values[5]}, '
                    f'weeks_passed={values[6]}  '
                    f'WHERE analyse_number={cell_to_update}'
                    )
        con.commit()
    ...
    def daily_graph_request(self, day_of_the_week):
        """
        ...
        for j in range(self.analyses_per_day * day_of_the_week, self.analyses_per_day * (day_of_the_week + 1)):
            sq = f"SELECT * FROM {name} WHERE analyse_number={j}"
            cur.execute(sq)
            values_tuple = cur.fetchone()
            return_dict[values_tuple[7]] = values_tuple[1]
        return return_dict

    def weekly_graph_request(self, day_of_the_week):
        """
        needed to make graphics
        :param day_of_the_week: number of the day we want to build graphic for
        :return: dictionary: time: percent
        ...
        for j in range(self.analyses_per_day * 7):
            sq = f"SELECT * FROM {name} WHERE analyse_number={j}"
            cur.execute(sq)
            values_tuple = cur.fetchone()
            return_dict[values_tuple[7]] = values_tuple[1]
